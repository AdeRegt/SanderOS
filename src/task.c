...
void createTask(unsigned char taskpointer, void (*main)()) {
    ...
    tasks[taskpointer].regs.esp = (unsigned long) main;//counter;//main;//mainTask.regs.esp;//allocPage() + 0x1000; // Not implemented here
...
void yield(unsigned char to) {
	...
	if(tasks[to].valid==0){
		...
		tasks[tpoint].curX = getCurX();
		tasks[tpoint].curY = getCurY();
